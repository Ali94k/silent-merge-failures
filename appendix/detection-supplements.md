# Appendix (candidate): detection supplements

Status: skeleton created by the ch.8 compression session (2026-09-09, A+ disposition; source text verbatim from merged draft-r17 except the flagged pointer edits) · Every item is marked **appendix-candidate** or **repo-cite (recommended)**; final print-appendix curation is Ali's end-of-wave ruling · Appendix letter assigned at curation · Pointer edits applied to moved text: the ch.5 flatten retargets (§5.3.5 → §5.3, §5.4.x → §5.4) — each flagged per item; no other wording changed · Narrative: none

## B.1 The detector cycle's numeric gate criteria — repo-cite (recommended)

Chapter home: §8.1 (Table T8.3 states the gates; criteria pre-registered in the plan [DetectorCycleGates]). Repo record: the committed cycle plan; `reports_detectors/FINDINGS.md`.
...
## B.2 The baseline repair's four defects — repo-cite (recommended)

Chapter home: §8.4.2. Repo record: `reports_detection/postfix{,2}/`. The fixes went into two separately labeled post-fix runs, not into the frozen suite. (The merged text listed these as two two-bullet lists, concatenated here: the first two are the mid-run finds left in place under the freeze, the last two the gaps the first post-fix run surfaced.)
...
## B.3 The tcurdt re-ruling — appendix-candidate

Chapter home: §8.4.2 (stub keeps the verdict and the retirement).
...
## B.4 The atam4j re-ruling — appendix-candidate

Chapter home: §8.4.2 (stub keeps the verdict and the cross-file false negative).
...
## B.5 The external run's deployment view — repo-cite (recommended)

Chapter home: §8.4.3 (Table T8.5 carries the values). Repo record: `reports_detection/phase2b/FINDINGS.md`.
...
## B.6 The worked example: D1's guard walk — appendix-candidate

Chapter home: §8.4.4 (stub keeps the outcome and the ruling). Pointer edit flagged: "(§5.3.5" → "(§5.3" per the flatten retargets; the flag-condition pointer "(§4.4.3)" as merged.
...
## B.7 Per-detector held-out prose — repo-cite (recommended)

Chapter home: §8.4.5 (Table T8.7 carries the outcomes). Repo record: `reports_detectors/FINDINGS.md`. Pointer edit flagged: none (all pointers as merged).
...
## B.8 The baseline rerun's split breakdown — repo-cite (recommended)

Chapter home: §8.4.6. Repo record: `reports_detectors/` §10.
...
## B.9 The lanes' latency components — repo-cite (recommended)

Chapter home: §8.4.7. Repo record: `reports_detectors/` §11.
...
## B.10 The five recall ceilings, full statements — appendix-candidate

Chapter home: §8.4.8 (compressed versions in print; the wildcard bullet's closing sentence is Ruling-C-protected wording and appears in both). Pointer edits flagged: "the recall denominators above" → "the recall denominators of §8.4.5" (the deictic pointed at chapter context the move removed).
