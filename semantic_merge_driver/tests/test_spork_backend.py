...
The first two tests are gated on `docker image inspect merge-tools/spork:0.5.0`
...
from core.backends.spork_backend import IMAGE, SporkBackend


def _docker_image_available():
    """Check whether the Spork Docker image is loadable on this host."""
    try:
        subprocess.run(
...
REQUIRES_SPORK_IMAGE = pytest.mark.skipif(
...
def test_clean_merge(tmp_path):
    ...
    outcome = SporkBackend().merge(base, ours, theirs)
