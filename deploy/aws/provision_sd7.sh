#!/usr/bin/env bash
# provision_sd7.sh — ON the EC2 instance (Ubuntu 24.04, x86_64) for the S-D7
# Stage-C rerun cycle (ISSUES #31 follow-on). Mirrors provision_sd5.sh: the
# two tool images (mergiraf 0.17.0 + refactoring-miner 2.4.0) are BUILT
# ON-INSTANCE from the repo's docker/ contexts — no images tarball.
#
# Expects in $HOME: repo_sd7.tar.gz  scenarios_sd7.tar.gz
# Usage: bash provision_sd7.sh   (then re-login for the docker group)
set -euo pipefail
...
mkdir -p "$REPO_DIR"
tar xzf "$HOME_DIR/repo_sd7.tar.gz" -C "$REPO_DIR"
tar xzf "$HOME_DIR/scenarios_sd7.tar.gz" -C "$MTC"
find "$MTC/data" -name "._*" -delete || true
for d in scenarios_semantic scenarios_semantic_ctl scenarios_detector_tune; do
  test -d "$MTC/data/$d" || { echo "ERR: data/$d missing"; exit 1; }
done
test -f "$MTC/reports_detection/postfix2/raw_results.json" \
  || { echo "ERR: postfix2 known-answer cache missing from repo archive"; exit 1; }
...
echo "==================================================================="
echo " S-D7 provision complete. Re-login (docker group), then:"
echo "   nohup bash thesis-projects/deploy/aws/run_sd7.sh > sd7_run.log 2>&1 &"
echo "==================================================================="
